# Xbox 商店遊戲對比工具 - V2 版本（已歸檔）

> **此版本已歸檔，請使用根目錄的 V3 版本。**  
> V3 修復了 TW 端批量查詢失效的問題，並移除 AUTH_TOKEN 必填限制。

支持增量更新、資料持久化、HTML 報表的改進版本。
...
打開 `xbox_compare_v2.py`，在 `# ===== 設定 =====` 區域修改：
...
# 測試模式：只掃描前 1 頁（預設）
python xbox_compare_v2.py --max-pages 1

# 完整掃描：掃描所有頁面
python xbox_compare_v2.py --max-pages 0
```
...
```bash
python xbox_compare_v2.py --multi-sort 0  # 快速，1-2 分鐘內完成
```
...
```bash
python xbox_compare_v2.py --multi-sort 1  # 完整但慢，30-60 分鐘
```
...
```bash
python xbox_compare_v2.py --browse-all 1 --max-pages 2
```
...
```bash
python xbox_compare_v2.py --browse-all 0 --db custom_db.db
```
...
```bash
python xbox_compare_v2.py --help
...
```bash
# 完整的增量更新
python xbox_compare_v2.py --browse-all 0 --multi-sort 1
```
